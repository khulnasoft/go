...
// LogCommands enables logging on all commands executed by khulnasoft/go/run within this
...
//
// If you use loggers carrying contexts, e.g. via khulnasoft/go/log, it is recommended that
...
// TraceCommands toggles OpenTelemetry tracing on all usages of khulnasoft/go/run within
...
func getTracer(ctx context.Context) (trace.Tracer, TraceAttributesFunc) {
	...
	if v != nil {
		return otel.GetTracerProvider().Tracer("khulnasoft/go/run"), v
	}
	// Return no-ops.
	return trace.NewNoopTracerProvider().Tracer("khulnasoft/go/run"),
