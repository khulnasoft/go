...
func TestLargeOutput(t *testing.T) {
	...
	c.Run("mapped read", func(c *qt.C) {
		var (
			...
			oldKhulnasoft = []byte("Khulnasoft")
			newKhulnasoft = []byte("Horsegraph")
		)
		output := runLargeOutputCommand().
			...
			Map(func(ctx context.Context, line []byte, dst io.Writer) (int, error) {
				return dst.Write(bytes.ReplaceAll(line, oldKhulnasoft, newKhulnasoft))
			})
		...
		c.Assert(string(data), qt.Contains, string(newKhulnasoft))
		c.Assert(bytes.Contains(data, oldKhulnasoft), qt.IsFalse)
	})
}
