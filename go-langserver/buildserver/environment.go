...
import (
	...
	"github.com/khulnasoft/go/go-langserver/gituri"
	"github.com/khulnasoft/go/go-langserver/langserver"
	"github.com/khulnasoft/go/go-langserver/pkg/lsp"
	"github.com/khulnasoft/go/go-lsp/lspext"
	"github.com/khulnasoft/go/jsonx"
)
...
func detectCustomGOPATH(ctx context.Context, fs ctxvfs.FileSystem) (gopaths []string) {
	...
	if paths := detectKhulnasoftGOPATH(ctx, fs); len(paths) > 0 {
		return paths
	}
...
// detectKhulnasoftGOPATH tries to detect monorepos which require their own custom
// GOPATH. We detect a .khulnasoft/config.json file with the following
// contents:
//
// 	{
// 	  "go": {
...
// 	}
//
// See the khulnasoftConfig struct documentation for more info.
//
// This is best-effort. If any errors occur or we do not detect a custom
// gopath, an empty result is returned.
func detectKhulnasoftGOPATH(ctx context.Context, fs ctxvfs.FileSystem) (gopaths []string) {
	cfg := readKhulnasoftConfig(ctx, fs)
...
// khulnasoftConfig is a struct representing the Go portion of the
// .khulnasoft/config.json file that may be placed in the repo root.
type khulnasoftConfig struct {
	...
	Go struct {
		...
		//
		// See https://github.com/khulnasoft/go/go-langserver#custom-gopaths-go-monorepos.
		GOPATH []string
		...
		//
		// See https://github.com/khulnasoft/go/go-langserver#vanity-import-paths.
		RootImportPath string
	} `json:"go"`
}

// readKhulnasoftConfig reads the .khulnasoft/config.json file from the
// repository root if it exists, otherwise an empty struct value (not nil).
func readKhulnasoftConfig(ctx context.Context, fs ctxvfs.FileSystem) *khulnasoftConfig {
	config := khulnasoftConfig{}
	b, err := ctxvfs.ReadFile(ctx, fs, "/.khulnasoft/config.json")
	if err != nil {
		return &config
	}
...
func determineRootImportPath(ctx context.Context, originalRootURI lsp.DocumentURI, fs ctxvfs.FileSystem) (rootImportPath string, err error) {
	...
	// If .khulnasoft/config.json specifies a root import path to use, then
