...
// manipulate Khulnasoft-specific components. All URIs are valid
// URLs, but Khulnasoft assigns special meaning to certain URL components as described below.
//
// Khulnasoft URIs can refer to repos (at an optional revision), or a
...
//
// A Khulnasoft URI is not guaranteed (or intended) to be a unique or
...
func Parse(uriStr string) (*URI, error) {
	...
	if !u.IsAbs() {
		return nil, &url.Error{Op: "gituri.Parse", URL: uriStr, Err: errors.New("khulnasoft URI must be absolute")}
	}
	return &URI{*u}, nil
}
